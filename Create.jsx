...
import { useNavigate, useLocation, createSearchParams, useHistory } from "react-router-dom"
...
import {Modal, Switch, Button, Alert, Space, Popconfirm, message } from "antd";
...
import CustomFinishModal from './FinishModal';

const Create = () => {
	const beforeUnloadWarning = useRef(true);
	useBeforeunload((event) => {
		console.log('beforeUnloadWarning.current', beforeUnloadWarning.current);
		if(beforeUnloadWarning.current === true){
			event.preventDefault();
			console.log("beforeunload happened!");
		}
	});
	const FACEBOOK_REGEXP=/^(https?:\/\/)facebook.com\/.+/
	const INSTA_REGEXP=/^(https?:\/\/)instagram.com\/.+/
	const TWITTER_REGEXP=/^(https?:\/\/)twitter.com\/([a-zA-Z0-9_]{1,15})\/?/
	const LINKEDIN_REGEXP=/^(https?:\/\/)linkedin.com\/(in\/[a-zA-Z0-9_-]+|company\/[a-zA-Z0-9_-]+)\/?/
	const YOUTUBE_REGEXP=/^(https?:\/\/)youtube.com\/[a-zA-Z0-9_-]+\/?/
	const SKYPE_REGEXP=/^skype:[a-zA-Z0-9_.]+(\?chat)?/
	const WHATSAPP_REGEXP=/\d+/
	const PINTEREST_REGEXP=/^(https?:\/\/)pinterest.com\/[a-zA-Z0-9_-]+\/?/
	...
	console.log("SOCIAL COUNTER+++",socialCounter)
   ...
   function rotateSize(width, height, rotation) {
	...
	function handleFinishClick() {
		try {
			if(loggerData?.isLoggedIn){
				...
				setShowFinishModal(true);
				// let modifiedUserData = modifyUserData();
				// let modifiedUserCTA = modifyUserCTA();
				// console.log("CTA FILLER MODIFIED...",modifiedUserCTA);
	...
	const validateTabberChange = (tabData, editMode) => {
		...
		if(editMode === true){
			...
			if (tabData === 'filter-social') {
				...
				if(allow === 3){
					setTabber(tabData);
					let modifiedUserDetails = modifyUserDetails();
			...
			if (tabData === 'filter-cta') {
				...
				/** END EMAIL VALIDATION */
				if(allow === 3){
					if(!userData?.visited?.social){
						return setWarning(prev => ({...prev, social:{status:true, message:""}}))
					}
					setTabber(tabData);
					let modifiedUserData = modifyUserSocial();
	...
	useEffect(() => {
		if(state?.template_id){
			setChosenTemplateId(state?.template_id);
			setLoggerData(prev => ({...prev, template_id:state?.template_id}));
	...
	console.log('CHOSEN DRAFT DATA', state?.chosen_draft_userData);
	console.log('\n\n\n\n\n\n\n')
		...
		try {
			...
			if (templates.length > 0) {
				...
				if (match) {
					...
					if (loggerData?.isLoggedIn) {
						if (isDraft) {
							console.log('DRAFT CHOSEN', state?.chosen_draft_userData);
							...
							if(loggerData?.userData?.email && loggerData?.userData?.fullName && loggerData?.userData?.designation){
								console.log('setting user Data equal to logger data')
	...
	const copyWithStyle = () => {
		try {
			if (!loggerData?.isLoggedIn) {
				...
				setLoggerData(prev => ({...prev, userData:{...prev.userData, ...modifiedUserData}}));
				setUserData(modifiedUserData);
				// before copying also save this to the users profile
				userServices.addUserData(loggerData?.user_id, loggerData?.email, loggerData?.access_token, modifiedUserData, chosenTemplateData?._id)
					.then(res => {})
					.catch((err)=>{});

				userServices.saveToDrafts(loggerData?.user_id, loggerData?.email, loggerData?.access_token, chosenTemplateData._id, chosenTemplateData.cta_data, sign_ref.current, { ...modifiedUserData })
					.then(res =>{})
					.catch((err) =>{})
	...
	async function handleSave() {
		try {
			...
			console.log('SAVING MODIFIED USER DATA TO DRAFTS', modifiedUserData);
			setLoggerData(prev => ({...prev, userData:{...prev.userData, ...modifiedUserData}}));
			setUserData(modifiedUserData);
			const result = await userServices.saveToDrafts(loggerData?.user_id, loggerData?.email, loggerData?.access_token, chosenTemplateData._id, chosenTemplateData.cta_data, sign_ref.current, { ...modifiedUserData });
	...
	function loginAndRedirect() {
		try {
			...
			// localStorage.setItem("template_data", sign_ref.current)
			// login
	...
	const handleGmailSignature = async () => {
		try {
			setLoading(true);
			beforeUnloadWarning.current = false;
			if (!loggerData?.isLoggedIn) {
				loginAndRedirect();
			} else {
				let modifiedUserData = modifyUserData();				
			...
			console.log('Some error occurred.', error);
			setLoading(false);
			beforeUnloadWarning.current = true;
	...
	async function handleRefresh() {
		try {
			...
			if(loggerData?.isLoggedIn && Object.keys(loggerData?.userData)?.length > 0 && loggerData?.userData?.fullName && loggerData?.userData?.email && loggerData?.userData?.designation && loggerData?.template_id){
				...
				console.log('Handle Refresh - CONDITION NOT MET')
			}
		} catch (error) {
	...
	useEffect(() => {
		if (state?.chosen_draft_data) {
			...
			console.log('USEEFFECT DRAFT', state?.chosen_draft_userData)
			fetchChosenTemplate(state.chosen_draft_template_id, true);
		} else {
			if(chosenTemplateId){
				console.log('**** GETTING TEMPLATE AS PER URL ****')
				fetchChosenTemplate(chosenTemplateId, false);
			}
			else if(loggerData?.template_id){
				fetchChosenTemplate(loggerData?.template_id, false);
			}
	...
	// 					const match = templates.filter(item => (item._id === template_id))[0];
	// 					if (match) {
	// 						setInitialValues({...match?.filler, facebook:'www.facebook.com/username'})
	// 					}
	// 				} else {
	// 					const template_data = await userServices.getTemplateById({ template_id: template_id });
	// 					if (template_data) {
	...
	// useEffect(()=>{
	// 	if(!loggerData?.isLoggedIn){
	// 		console.log('RESETING INITIAL VALUES')
	// 		resetInitialValues();
	// 		// if(loggerData?.template_id){
	// 		// 	fetchChosenTemplate(loggerData?.template_id, false);
	// 		// }
	// 	}
	// 	else if (loggerData?.isLoggedIn){
	// 		if(loggerData?.userData?.email && loggerData?.userData?.fullName && loggerData?.userData?.designation){
	// 			console.log('setting user Data equal to logger data');
	// 			setUserData(prev => ({...prev, ...loggerData?.userData}));
	// 		}
	...
	useEffect(()=>{
			if(userData?.facebook === ""){
				...
				(socialCounter?.indexOf("facebook") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"facebook"]))
			}
			if(userData?.linkedIn === "" ){
				setSocialChecker(prev =>({...prev, linkedIn:false}))
				socialCounter?.indexOf("linkedIn") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "linkedIn"))) 
				...
				(socialCounter?.indexOf("linkedIn") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"linkedIn"]))
			}
			if(userData?.twitter === "" ){
				setSocialChecker(prev =>({...prev, twitter:false}))
				socialCounter?.indexOf("twitter") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "twitter"))) 
				...
				(socialCounter?.indexOf("twitter") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"twitter"]))
			}
			if(userData?.instagram === "" ){
				setSocialChecker(prev =>({...prev, instagram:false}))
				socialCounter?.indexOf("instagram") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "instagram"))) 
				...
				(socialCounter?.indexOf("instagram") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"instagram"]))
			} 
			if(userData?.youtube === "" ){
				setSocialChecker(prev =>({...prev, youtube:false}))
				socialCounter?.indexOf("youtube") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "youtube"))) 
				...
				(socialCounter?.indexOf("youtube") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"youtube"]))
			} 
			if(userData?.whatsapp === "" ){
				setSocialChecker(prev =>({...prev, whatsapp:false}))
				socialCounter?.indexOf("whatsapp") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "whatsapp"))) 
				...
				(socialCounter?.indexOf("whatsapp") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"whatsapp"]))
			} 
			if(userData?.skype === "" ){
				setSocialChecker(prev =>({...prev, skype:false}))
				socialCounter?.indexOf("skype") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "skype"))) 
			} else if(userData?.skype){
				setSocialChecker(prev =>({...prev, skype:true}));
				(socialCounter?.indexOf("skype") === -1 && socialCounter?.length < 5) && setSocialCounter(prev => ([...prev,"skype"]))
			} 
			if(userData?.pinterest === "" ){
				setSocialChecker(prev =>({...prev, pinterest:false}))
				socialCounter?.indexOf("pinterest") !== -1 && setSocialCounter(prev => (prev.filter(item => item !== "pinterest"))) 
			} else if(userData?.pinterest){
												...
												</div>
												{
													image && (
														<div className="image-preview-blk">
															...
															<img onClick={()=>{setIsModalOpen(true)}} src={src} alt="preview" width={100} height={100} style={{borderRadius:'10px', boxShadow:'4px 4px 12px 3px rgba(0,0,0,0.12)', objectFit:"cover"}} />
														</div>
													)
												}
											</div>
									...
									<div className="create-template-categories  animate__animated animate__fadeIn login-form">
										<form method="post" action="">
											...
											<label htmlFor="whatsapp-check" id="custom-whatsapp-check"></label>
												...
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
														if(WHATSAPP_REGEXP.test(e.target?.value?.slice(0,100))){
							...
							<div className="create-template-right tabber-content filter-cta right">
								<div className="create-template-right-inner">
									...
									<div className="create-template-details">
										...
										<div className="template-detail-btn">
											<span onClick={() => { validateTabberChange('filter-cta', true) }} data-filter="filter-cta" className="anchor-tabber site-btn-second-reverse">Back</span>
